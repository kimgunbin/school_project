...
//            contentText.setText(item.getContent());
    ...
    public class LeftImageViewHolder extends RecyclerView.ViewHolder {
        ...
        public void setItem(ChatItem item, Context context) {
            MultiTransformation option = new MultiTransformation(new CenterCrop(),
                    new RoundedCorners(8));

            Glide.with(context).load(item.getContent()).apply(RequestOptions.bitmapTransform(option))
                    .into(image);
            nameText.setText(item.getName());
            sendTimeText.setText(item.getSendTime());
        }
    ...
    public class RightImageViewHolder extends RecyclerView.ViewHolder {
        ...
        public void setItem(ChatItem item, Context context) {
            ...
            Glide.with(context).load(item.getContent())
                    .apply(RequestOptions.bitmapTransform(option)).into(image);
            sendTimeText.setText(item.getSendTime());
        }
    }
}
